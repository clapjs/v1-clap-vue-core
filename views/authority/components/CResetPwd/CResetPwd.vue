<template>
    <a-modal :visible="visible" title="密码重置" centered :mask-closable="false" :closable="false" dialogClass="noDrag" @cancel="cancel" @ok="doResetPwd">
        <a-form-model ref="form" labelAlign="left" :model="form" :rules="rules" :label-col="{span: 4}" :wrapper-col="{span: 20}">
            ...
            <a-form-model-item label="验证码" v-if="forget" prop="verifyCode">
                <a-input-search v-model="form.verifyCode">
                    <a-button slot="enterButton" type="primary">获取验证码</a-button>
                </a-input-search>
            </a-form-model-item>
            <a-form-model-item label="原密码" prop="userPwd" v-else>
...
export default {
    ...
    data(){
        ...
        return{
            ...
            rules: {
                ...
                verifyCode:[{required: true, message: '必填', trigger: 'change'}],
    ...
    mounted() {
        this.form.userCode = !!this.$ls.get('USER_REMEMBER')?this.$ls.get('USER_CODE'):'';
    },
    methods:{
        cancel(){
