...
const loadRouterAsync=async (Router,application)=>{
    ...
    ifExist('index')&&Router.addRoute({name: 'index', path: '/', component: CLayout, redirect : () => {return 'dash'}});
    ifExist('dash')&& Router.addRoute('index',{name: 'dash', path: '/dash', component: () => import('@/views/dash')});
    for (let route of routes) {
        if (route.routePath) {
            ...
            switch (route.location) {
                case 'Layout':
                    ifExist(route.routeName)&& Router.addRoute('index',RouterConfig);
                    break;
                case'Root':
                    ifExist(route.routeName)&& Router.addRoute(RouterConfig);
                    break;
                default:
                    break;
            }
    ...
    ifExist('other')&&Router.addRoute({name:'other',path: '*', redirect: '/404'});
}
...
const app = {
    ...
    actions: {
        ...
        ToggleMenu({commit,state,dispatch}, menu) {
            menu=menu?menu:{key: 'dash', title: '首页', routeName: 'dash', controlType: 'Group', idOrgan: '', organs: [], closable: false}
